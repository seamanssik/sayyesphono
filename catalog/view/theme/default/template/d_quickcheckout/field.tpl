	...
	<% if(model[model.config.id][f.id] !== undefined || f.type == "heading" || f.type == "label"){ %>
		<% if(f.type){ %>
	    	...
	    	<% }else if(f.type == "checkbox"){ %>
			    ...
			    <div id="<%= model.config.id %>_<%= f.id %>_input" 
			    class="checkbox-input form-group sort-item <%= display ? '' : 'hidden' %> <%= f.class ? f.class : '' %> <%= require ? '' : '' %>"
			    data-sort="<%= f.sort_order %>">
			    	...
			    	<% if(f.options){ %>
		    			...
		    			<div class="col-xs-12">
					      <label for="<%= model.config.id %>_<%= f.id %>" >
					          ...
					          <span class="text" <%= f.tooltip ? 'data-toggle="tooltip"' : '' %> title="<%= f.tooltip %>"><%= htmlDecode(option.name) %></span>
					        </label>
					    </div>
					    <% i++ %>
					    <% })%>
	    	...
	    	<% }else if(f.type == "select"){ %>
				...
				<div id="<%= model.config.id %>_<%= f.id %>_input" 
		        class="select-input form-group  sort-item <%= display ? '' : 'hidden' %> <%= f.class ? f.class : '' %> <%= require ? '' : '' %>"
		        data-sort="<%= f.sort_order %>">
					<div class="col-xs-12">
<!--		        <div class="<%= col_left %>">-->
		          <label class="control-label checkout-form__label" for="<%= model.config.id %>_<%= f.id %>">
...
<!--		        <div class="<%= col_right %>">-->
		          <select name="<%= model.config.id %>.<%= f.id %>" 
		            <%= require ? 'required' : '' %> 
		            id="<%= model.config.id %>_<%= f.id %>"
		            class="checkout-form__select form-control <%= require ? 'required' : 'not-required' %> <%= f.id %>"  autocomplete="<%= autocomplete %>"  >
...
<!--		        </div>-->
			...
			<% }else if(f.type == "tel"){ %>
			      <div id="<%= model.config.id %>_<%= f.id %>_input" 
			        class="text-input form-group  sort-item <%= display ? '' : 'hidden' %> <%= f.class ? f.class : '' %> <%= require ? '' : '' %>"
			        data-sort="<%= f.sort_order %>">
					  <div class="col-xs-12">
<!--			        <div class="<%= col_left %>">-->
			          <label class="control-label checkout-form__label" for="<%= model.config.id %>_<%= f.id %>">
...
<!--			        <div class="<%= col_right %>"> -->
			          <input type="<%= f.type %>" 
			            ...
			            id="<%= model.config.id %>_<%= f.id %>" 
			            value="<%= model[model.config.id][f.id] %>" 
			            class="checkout-form__input form-control <%= f.mask ? 'qc-mask': '' %> <%= require ? 'required' : 'not-required' %> <%= f.id %>"
			            autocomplete="<%= autocomplete %>" 
			            <% if(f.mask){ %>
			            qc-mask="<%= f.mask %>"
			            <% } %> 
