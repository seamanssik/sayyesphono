...
<% }else{ %>
	...
	<div class="login-btn-group checkout-login__group">
		<div class="btn-group btn-group-justified">
		<% if(parseInt(model.config.option.guest.display)){ %>
			<div class="btn-group checkout-login__btn" role="group">
				<label class="checkout-login__label btn <%= model.account == 'guest' ? 'btn-primary active' : 'btn-default' %>">
		...
		<% if(parseInt(model.config.option.register.display)){ %>
			<div class="btn-group checkout-login__btn" role="group">
				<label class="checkout-login__label btn <%= model.account == 'register' ? 'btn-primary active' : 'btn-default' %> <%= parseInt(model.config.option.register.display) ? '' : 'hidden' %>">
		...
		<% if(config.design.login_style == 'popup' && parseInt(model.config.option.login.display)){ %> 
			<div class="btn-group checkout-login__btn" role="group">
				<button type="button" class="btn btn-default checkout-login__button" data-toggle="modal" data-target="#modalSignIn"><%= model.config.option.login.title %></button>
			</div>
		<% } %>
		</div>
	</div>
