...
class ModelCatalogNews extends Model {
    ...
    public function getNewsPagination($data = array()) {
        $sql_get_category = "SELECT ntc.ncategory_id FROM " . DB_PREFIX . "sb_news_to_ncategory ntc";

        $sql_get_category .= " LEFT JOIN " . DB_PREFIX . "sb_news n ON (n.news_id = ntc.news_id)";

        $sql_get_category .= " WHERE ntc.news_id = '" . $data['news_id'] . "'";

        if (!empty($data['ncat'])) {
            $sql_get_category .= " AND ntc.ncategory_id = '" . $data['ncat'] . "'";
        }

        $sql_get_category .= " AND n.status = '1'";

        $query_get_category = $this->db->query($sql_get_category);

        if ($query_get_category) {
            $ncategory_id = $query_get_category->row['ncategory_id'];

            $sql_get_news = "SELECT ntc.news_id AS news_id FROM " . DB_PREFIX . "sb_news_to_ncategory ntc";

            $sql_get_news .= " LEFT JOIN " . DB_PREFIX . "sb_news n ON (n.news_id = ntc.news_id)";

            $sql_get_news .= " WHERE ntc.ncategory_id = '" . $ncategory_id . "'";

            $sql_get_news .= " AND n.status = '1'";
