...
class ControllerDQuickcheckoutCart extends Controller {
    ...
    public function prepare($json){
        ...
        $json['countProducts'] = num2word($this->cart->countProducts(), array( sprintf($this->language->get('text_cart_1'), $this->cart->countProducts()), sprintf($this->language->get('text_cart_2'), $this->cart->countProducts()), sprintf($this->language->get('text_cart_3'), $this->cart->countProducts())), $this->cart->countProducts());
        ...
        foreach ($products as $product) {
            ...
            if ($product['image']) {
                $image = $this->model_tool_image->cropsize($product['image'], $this->session->data['d_quickcheckout']['design']['cart_image_size']['width'], $this->session->data['d_quickcheckout']['design']['cart_image_size']['height']);
            } else {
                $image = '';
            }
            ...
            foreach ($product['option'] as $option) {
                ...
                if ($option['option_id'] == 17) {
                    $additions[] = array(
                        'name'      => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value),
                        'price'     => $option['price_prefix'] . '' . (float)$option['price'],
                        'symbol'    => $this->currency->getSymbolRight($this->session->data['currency'])
                    );
                }

                if ($option['option_id'] != 18 && $option['option_id'] != 16 && $option['option_id'] != 15) {
                    $option_data[$option['option_id']] = array(
                        'name' => $option['name'],
                ...
                if ($option['option_id'] == 15) {
                    $option_data[$option['option_id']] = array(
                        'name'      => $option['name'],
                        'value'     => date('d', strtotime($value)) . ' ' . rus_months($this->session->data['language'], date('m', strtotime($value))) . ' ' . date('Y', strtotime($value)) . ' ' . $this->language->get('year'),
                        'additions' => false
                    );
                }

                if ($option['option_id'] == 23) {
                    $option_data[$option['option_id']] = array(
                        'name'      => $option['name'],
                        'value'     => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value) . ' ' . $this->language->get('year'),
                        'additions' => $additions
                    );
                }
                ...
                if ($option['option_id'] == 18) {
                    if ($option['image']) {
                        $image = $this->model_tool_image->cropsize($option['image'], $this->session->data['d_quickcheckout']['design']['cart_image_size']['width'], $this->session->data['d_quickcheckout']['design']['cart_image_size']['height']);
                    }
                }
            }
