...
class ControllerNewsNcategory extends Controller {
	...
	protected function getPageContent($settings) {
		...
		foreach ($results as $result) {
			...
			$da = '<span>' . date('d', strtotime($result['date_added'])) . '</span><span>' . date('m', strtotime($result['date_added'])) . '</span>';
			...
			if (in_array("category", $elements)) {
				...
				if ($cats) {
					...
					foreach($cats as $catid) {
						...
						if ($catinfo) {
							if ($comma) {
								$category .= ', <a href="'.$this->url->link('news/ncategory', 'ncat=' . $catinfo['ncategory_id']).'"><span>' . $catinfo['name'] . '</span></a>';
							} else {
								$category .= '<a href="'.$this->url->link('news/ncategory', 'ncat=' . $catinfo['ncategory_id']).'"><span>' . $catinfo['name'] . '</span></a>';
							}
							$comma++;
			...
			$data['article'][] = array(
				...
				'thumb'       => $image,
				'date_added'  => $da,
				'date_year'	  => date('Y', strtotime($result['date_added'])),
				'du'          => $du,
				'author'      => $author,
				'author_id'   => $author_id,
				'author_link' => $author_link,
				'description' => strip_tags(html_entity_decode($result['description2'], ENT_QUOTES, 'UTF-8')),
				'button'      => $button,
				'custom1'     => $custom1,
				'custom2'     => $custom2,
				'custom3'     => $custom3,
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				'name'  => $result['name'],
				'href'  => $this->url->link('news/ncategory', 'ncat=' . $result['ncategory_id'])
			);
		}
