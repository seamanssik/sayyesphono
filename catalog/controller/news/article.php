...
class ControllerNewsArticle extends Controller {
	...
	protected function getPageContent($news_info) {
		...
		if ($cats) {
			...
			foreach($cats as $catid) {
				...
				if ($catinfo) {
					...
					$categories[] = $this->url->link('news/ncategory', 'ncat=' . $catinfo['ncategory_id']);
		...
		$data['category_link'] = array_pop($categories);
		...
		$data['date'] = date("d", strtotime($news_info['date_added'])) . '<br>' . date("m", strtotime($news_info['date_added']));
		$data['year'] = date("Y", strtotime($news_info['date_added']));
		...
		if ($news_info['image']) {
			$data['thumb'] = $this->model_tool_image->cropsize($news_info['image'], $bwidth, $bheight);
			$data['popup'] = $this->model_tool_image->resize($news_info['image'], 600, 600);
			$data['share'] = $this->model_tool_image->resize($news_info['image'], 200, 200);
		} else {
			$data['thumb'] = '';
			$data['share'] = '';
		}
		...
		foreach ($gal_results as $result) {
			...
			$data['gallery_images'][] = array(
				...
				'thumb' => $this->model_tool_image->cropsize($result['image'], $news_info['gal_thumb_w'], $news_info['gal_thumb_h'])
			);
		}
		...
		$data['news_pagination'] = false;

		$filter_pagination = array();

		$filter_pagination['news_id'] = (int)$data['news_id'];

		if (!empty($this->request->get['ncat'])) {
			$ncat = explode('_', $this->request->get['ncat']);

			$filter_pagination['ncat'] = array_pop($ncat);
		}

		$url = false;

		$category_article = $this->model_catalog_news->getNcategoriesbyNewsId($data['news_id']);

		if ($category_article) {
			$category_id = array_pop($category_article)['ncategory_id'];
			$url = 'ncat=' . $category_id;
		}

		$results_pagination = $this->model_catalog_news->getNewsPagination($filter_pagination);

		if ($results_pagination) {
			$news_pagination = array();

			foreach ($results_pagination as $result_pagination) {
				$news_pagination[] = $result_pagination['news_id'];
			}
		}
		...
		if (!empty($i_PrevID)) {
			$data['pagination_prev'] = $this->url->link('news/article', $url . '&news_id=' . $i_PrevID);
		}

		$data['pagination_next'] = false;
		if (!empty($i_NextID)) {
			$data['pagination_next'] = $this->url->link('news/article', $url . '&news_id=' . $i_NextID);
		}
