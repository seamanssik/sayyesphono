...
class ControllerInformationContact extends Controller {
	...
	public function sendRequest() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateRequest()) {
			...
			$mail->setSubject(html_entity_decode('Заявка на сотруднечесто', ENT_QUOTES, 'UTF-8'));
			$mail->setHtml('<p><b>Имя:</b> ' . $this->request->post['name'] . '</p>' . '<p><b>Телефон:</b> ' . $this->request->post['tel'] . '</p>' . '<p><b>Дата заявки:</b> ' . date('d.m.Y h:i') . '</p>');
			$mail->send();
	...
	public function sendQuestion() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateQuestion()) {
			...
			$mail->setFrom($this->config->get('config_email'));
			$mail->setSender(html_entity_decode($this->request->post['name'], ENT_QUOTES, 'UTF-8'));
			$mail->setSubject(html_entity_decode('Связаться со стилистом', ENT_QUOTES, 'UTF-8'));
			$mail->setHtml('<p><b>Имя:</b> ' . $this->request->post['name'] . '</p>' . '<p><b>Телефон:</b> ' . $this->request->post['telephone'] . '</p>' . '<p><b>Дата заявки:</b> ' . date('d.m.Y h:i') . '</p>');
			$mail->send();
	...
	public function index() {
		...
		$data['c_email'] = $this->config->get('config_email');
	...
	protected function validateRequest() {
		...
		if ((utf8_strlen($this->request->post['tel']) < 13) || (utf8_strlen($this->request->post['tel']) > 21)) {
	...
	protected function validateQuestion() {
		...
		if ((utf8_strlen($this->request->post['telephone']) < 13) || (utf8_strlen($this->request->post['telephone']) > 21)) {
