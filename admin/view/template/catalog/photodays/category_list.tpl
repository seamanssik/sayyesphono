          ...
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($category['photodays_category_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $category['photodays_category_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $category['photodays_category_id']; ?>" />
                    <?php } ?></td>
                  <td class="text-left"><?php echo $category['name']; ?></td>
                  <td class="text-right"><?php echo $category['sort_order']; ?></td>
                  <td class="text-left">
                    <?php if (in_array(0, $category['category_store'])) { ?>
                      <p style="margin: 0"><span class="label label-primary"><?php echo 'Основной магазин'; ?></span></p>
                    <?php } ?>
                    <?php foreach ($category['stores'] as $store) { ?>
                      <?php if (in_array($store['store_id'], $category['category_store'])) { ?>
