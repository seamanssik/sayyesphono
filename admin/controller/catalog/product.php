    ...
    // OCFilter end
		...
		foreach ($product_options as $product_option) {
			...
			$data['product_options'][] = array(
				...
				'multiple_date'        => isset($product_option['multiple_date']) ? $product_option['multiple_date'] : '',
		...
		// Image
		if (isset($this->request->post['image_history'])) {
			$data['image_history'] = $this->request->post['image_history'];
		} elseif (!empty($product_info)) {
			$data['image_history'] = $product_info['image_history'];
		} else {
			$data['image_history'] = '';
		}

		if (isset($this->request->post['image_history']) && is_file(DIR_IMAGE . $this->request->post['image_history'])) {
			$data['thumb_history'] = $this->model_tool_image->resize($this->request->post['image_history'], 100, 100);
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['image_history'])) {
			$data['thumb_history'] = $this->model_tool_image->resize($product_info['image_history'], 100, 100);
		} else {
			$data['thumb_history'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		// Reviews
		if (isset($this->request->post['product_reviews'])) {
			$product_reviews = $this->request->post['product_reviews'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_reviews = $this->model_catalog_product->getProductReviews($this->request->get['product_id']);
		} else {
		...
		foreach ($product_reviews as $product_review) {
			if (is_file(DIR_IMAGE . $product_review['image'])) {
				$image = $product_review['image'];
				$thumb = $product_review['image'];
			} else {
				$image = '';
				$thumb = 'no_image.png';
			}
			...
			$data['product_reviews'][] = array(
				...
				'expert' 	 => $product_review['expert'],
				'position'   => $product_review['position'],
				'sort_order' => $product_review['sort_order']
			);
		}

		// Studio
		if (isset($this->request->post['product_studios'])) {
			$product_studios = $this->request->post['product_studios'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_studios = $this->model_catalog_product->getProductStudios($this->request->get['product_id']);
		} else {
			$product_studios = array();
		}

		$data['product_studios'] = array();

		foreach ($product_studios as $product_studio) {
			if (is_file(DIR_IMAGE . $product_studio['image'])) {
				$image = $product_studio['image'];
				$thumb = $product_studio['image'];
			} else {
				$image = '';
				$thumb = 'no_image.png';
			}
			...
			$data['product_studios'][] = array(
				'image'      => $image,
				'thumb'      => $this->model_tool_image->resize($thumb, 100, 100),
				'text' 	 	 => $product_studio['text'],
				'title' 	 => $product_studio['title'],
				'name'   	 => $product_studio['name'],
				'sort_order' => $product_studio['sort_order']
			);
		}
