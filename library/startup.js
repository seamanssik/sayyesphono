...
$(document).on('click', '.showroom-total__order', function (e) {
    if ( is.not.mobile() ) {
        $('body').animate({scrollTop: _body.find('.photoday-order').first().offset().top - 90}, 'slow');
    } else {
        $('body').animate({scrollTop: _body.find('.photoday-order').first().offset().top}, 'slow');
    }
...
function scrollHeader() {
    if(is.desktop() || is.tablet()){
        ...
        if (parseInt(_scroll) > parseInt($('header').height() + $('.n-container').height()) + 40) {
            $('.h-fixed').addClass('show');
        } else {
            $('.h-fixed').removeClass('show');
        }
    }
}
...
$(document).on('click', '*[data-toggle="openCloseText"]', function () {
    var _this = $(this),
        _element = _this.attr('data-element'),
        _short_text = _this.attr('data-short'),
        _short_action = _this.attr('data-action'),
        _height = _this.attr('data-height');

    if (!_this.hasClass('active')) {
        _this.children('span').text(_short_text);
        $("#" + _element).animate({height: $("#" + _element).children('div').height()}, 1000);
    } else {
        _this.children('span').text(_short_action);
        $("#" + _element).animate({height: parseInt(_height) + 'px'}, 1000);
    }
